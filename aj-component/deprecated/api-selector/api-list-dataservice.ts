...
export default {
    ...
    methods: {
        getDataService(): void {
            get(`${this.API}/datasource`, (j: RepsonseResult) => {
        ...
        loadTablesCfg(item: any, callback: Function): void {
            ...
            get(`${this.API}/data_service?datasourceId=${datasourceId}`, (j: RepsonseResult) => {
                if (j.data) {
                    this.list = j.data;
                    let data: any[] = [];
                    let i: number = 0;
        ...
        appendDataServiceDetail(data: any): void {
            if (!data.isLoaded) {
                // 预先加载
                let tableFields: any[] = [];
                get(`${this.API}/data_service/getAllFieldsByDataSourceAndTablename?datasourceId=${data.cfg.datasourceId}&tableName=${data.cfg.tableName}`,
                    (j: RepsonseResult) => {
                        console.log(j)
                        //@ts-ignore
