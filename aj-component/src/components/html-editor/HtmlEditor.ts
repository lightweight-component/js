...
export default defineComponent({
    ...
    data(): any {
        return {
            ...
            iframeEl: null as HTMLIFrameElement | null,
            sourceEditor: null,
        };
    },
    mounted(): void {
        this.iframeEl = this.$el.querySelector('iframe') as HTMLIFrameElement;
        this.sourceEditor = this.$el.querySelector('textarea') as HTMLTextAreaElement;

        this.iframeEl.contentWindow.onload = (ev: Event) => { // 这个方法只能写在 onload 事件里面， 不写 onload 里还不执行
            this.iframeDoc = (this.iframeEl.contentWindow as Window).document;
            ...
            new MutationObserver((mutationsList: MutationRecord[], observer: MutationObserver) => {// 监听DOM树变化
                if (!this.isShowCode) {
                    ...
                    this.$emit('onchange', this.sourceEditor.value);
                }

                this.$emit('update:modelValue', this.iframeDoc.body.innerHTML);
            }).observe(this.iframeDoc.body, { attributes: true, childList: true, subtree: true, characterData: true });

            // this.modelValue && this.setIframeBody(this.modelValue);
    ...
    methods: {
        ...
        */
        ...
        createLink(): void {
            const result: string | null = prompt("请输入 URL 地址");
        ...
        saveRemoteImage2Local(): void {
            ...
            for (let i = 0, j = arr.length; i < j; i++) {
                const imgEl: HTMLImageElement = arr[i],
                    src: string | null = imgEl.getAttribute('src');
        ...
        onCmdClk(ev: Event): void {
            const el: HTMLElement = ev.target as HTMLElement,
                clsName = el.className.split(' ').shift();
        ...
        onFontsizeChoserClk(ev: Event): void {
            const el: HTMLElement = ev.target as HTMLElement,
                els = (ev.currentTarget as HTMLElement).children;
        ...
        onFontfamilyChoserClk(ev: Event): void {
            ...
            const menuPanel: HTMLElement = el.parentNode as HTMLElement;
    ...
    watch: {
        ...
        // },
        modelValue(newHtml: string, oldHtml: string): void {
            // if (!html)
            //     html = '';
            if (!oldHtml) {
                // 当没有值的时候输入，就是在初始化的时候（第一次）
                this.setIframeBody(newHtml);
            }
        },
...
function onImagePaste(ev: ClipboardEvent): void {
    ...
    // 监听请求完成
    xhr.addEventListener('load', (event: ProgressEvent<XMLHttpRequestEventTarget>) => {
        if (xhr.status >= 200 && xhr.status < 300) {
            const result = JSON.parse(xhr.responseText);
            console.log('Success:', result);
            // alert('File uploaded successfully!');
            
            this.format("insertImage", result.data.url);
        } else {
    ...
    xhr.open('POST', this.uploadImageActionUrl); // 替换为你的实际上传接口地址
