...
export default defineComponent({
  ...
  methods: {
    ...
    getList(callback?: (data: any) => void): void {
      XhrFetch.get(`${this.api}/datasource?start=0&`, (j) => {
    ...
    create(): void {
      ...
      form.validate((valid: boolean) => {
        if (valid)
          XhrFetch.post(`${this.api}/datasource`, this.form.data, j => {
            if (j.status === 1) {
    ...
    update(): void {
      ...
      XhrFetch.put(`${this.api}/datasource`, entity, j => {
        if (j.status === 1)
    ...
    del(id: number, name: string): void {
      this.$Modal.confirm({
        ...
        onOk: () => {
          XhrFetch.del(this.api + id, (j: any): void => {
    ...
    test(): void {
      XhrFetch.get(`${this.api}/datasource/test/` + this.activedItem, (j) => {
        if (j.status)
