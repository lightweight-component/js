...
export default defineComponent({
  ...
  data(): RolePanel {
    return {
      simpleApi: `${window.config.iamApi}/common_api`,
      roleApi: `${window.config.iamApi}/permission`,
      permissionApi: `${window.config.iamApi}/module_permission`,
  ...
  methods: {
    ...
    editRole(): void {
      ...
      XhrFetch.get(`${this.simpleApi}/role/${this.contextData.id}`, (j: any) => {
        if (j.status) {
    ...
    */
    ...
    delRole(): void {
      ...
      this.$Modal.confirm({
        ...
        onOk: () => {
          XhrFetch.del(`${this.roleApi}/role/${this.contextData.id}`, (j: any) => {
            if (j.status) {
    ...
    refreshRoleList(): void {
      XhrFetch.get(`${this.roleApi}/role_tree`, (j: any) => {
        if (j.status) {
  ...
  watch: {
    currentRole(currentRole): void {
      if (currentRole && currentRole.id) {
        XhrFetch.get(`${this.permissionApi}/permission_list_by_role/${currentRole.id}`, (j: any) => {
          if (j.status) {
