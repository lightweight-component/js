  ...
  <div>
    ...
    <Table row-key="id" :columns="columns16" :data="data" border size="small">
      <template v-slot:action="{ row }">
        <Button size="small" icon="md-add" type="success"
          @click="createTreeNode(row.name, row.id, row.parentId)">新建</Button>
...
export default {
  ...
  methods: {
    getData() {
      ...
      XhrFetch.get(`${this.api.list}`, (j: ApiResponseResult) => {
    ...
    save(): void {
      if (this.isUpdate) {
        ...
        XhrFetch.put(`${this.api.update}`, (j: RepsonseResult) => {
          if (j.status) {
            this.getData()
        ...
        XhrFetch.post(`${this.api.create}`, (j: RepsonseResult) => {
          if (j.status) {
            this.getData()
            this.$Message.success(j.message)
          } else
    ...
    delTreeNode({ id }) {
      XhrFetch.del(`${this.api.delete}`, (j: RepsonseResult) => {
        if (j.status) {
          this.getData()
          this.$Message.success(j.message)
        } else
