<template>
  <div class="container">
    ...
    <div class="right">
      <nav>
        MyAPI
        <Icon type="md-help-circle" style="margin: 10px 10px 0 10px; float: right; cursor: pointer" size="19"
      ...
      <Tabs class="mainTab" name="mainTab" :value="activeTab" :animated="false" type="card" @on-click="ifAdd">
        <TabPane v-for="tab in mainTabs" :key="tab.label" :label="tab.label" :name="tab.name" :index="tab.index"
...
<style>
.mainTab>.ivu-tabs-bar>.ivu-tabs-nav-container>.ivu-tabs-nav-wrap>.ivu-tabs-nav-scroll {
  padding-left: 20px;
  margin-top: 3px;
}
</style>
