  ...
  <div class="config-table">
    <div :class="{ main: true, small: isShowLivePerview && !belowPerview, 'below-perview': belowPerview }">
      <span style="float:right">
        ...
        <Button :disabled="isDisabled" icon="ios-create-outline" type="primary"
          @click="$parent.saveOrUpdate">保存</Button>
      </span>

      <Button class="rightMargin" icon="md-list" @click="isShowFieldsSelect = true"
        title="根据数据库各个字段导入列表的元数据">选择数据库</Button>
      ...
      <Button class="rightMargin" :disabled="$parent.editIndex != -1" icon="md-add"
        @click="$parent.addRow">新增字段</Button>
      ...
      <ul class="table">
        ...
        <draggable class="content-panel" tag="section" handle=".handleSort" v-model="fields" item-key="index"
          chosenClass="chosen" forceFallback="true" group="people" animation="1000">
            ...
            <li>
              <div class="action">
                ...
                <div v-else>
                  ...
                  <a @click="isShowMoreAttrib = true; currentRow = element;" title="进入详细的属性（attribute）配置">更多</a>
                  <Divider type="vertical" />
                  <a @click="moreAttrib(row, index)" class="handleSort" style="cursor: move;color:green"
                    title="点击鼠标不放拖动以排序">排序</a>
                  <Divider type="vertical" />
              ...
              <div class="isShow checkbox-width">
                <Checkbox size="small" v-model="element.isShow" />
              </div>
              <slot name="table-fields" :item="element" />
            </li>
          </template>
        </draggable>
      </ul>
    ...
    <Modal v-model="isShowFieldsSelect" title="选择数据库字段" width="1000"
      @on-ok="$parent.fieldsToCfg($refs.DataModelSelector.getSelected())">
...
import DataModelSelector from "../common/data-model-selector.vue";
