    ...
    methods: {
        ...
        saveOrUpdate(this: Common): void {
            ...
            if (this.id) {
                ...
                XhrFetch.put(`${window.config.dsApiRoot}/common_api/ds_widget_config`, valueObj, (j: ApiResponseResult) => {
                    if (j.status)
                ...
                XhrFetch.post(`${window.config.dsApiRoot}/common_api/ds_widget_config`, valueObj, (j: RepsonseResult) => {
                    if (j.status) {
                        this.$Message.success('创建成功');
                        setTimeout(() => this.id = j.data.newlyId, 800);
                    } else
                        this.$Message.warning(j.message);
                });
        },
         ...
         * 新增
         */
        addRow_(this: Common, row: any): void {
