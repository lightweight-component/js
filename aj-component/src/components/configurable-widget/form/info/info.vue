<template>
  <ConfigTable ref="configTable" class="form-factory">
    ...
    <template v-slot:live-perview>
      <FormLoader ref="LivePerview" :is_show_btns="false" />
    </template>
    ...
    <template v-slot:more-attrib="scope">
      <MoreAttrib v-if="scope.row" :row="scope.row" :ext-attribs="scope.row.ext_attribs" />
    </template>
    ...
    <Modal v-model="isShowPerview" title="预览" width="800" ok-text="关闭" cancel-text="">
      <FormLoader ref="preview" is-Show-Info-Btn />
    </Modal>
  </ConfigTable>
</template>
