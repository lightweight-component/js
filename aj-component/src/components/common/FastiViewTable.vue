...
export default defineComponent({
  ...
  methods: {
    getData(): void {
      ...
      this.listApiUrl_ = 'http://localhost:8088/robot_api/common_api/ds_widget_config/page_no?allow=1&q_type=LIST';

      this.listApiUrl_ && get(`${this.listApiUrl_}&pageNo=${this.list.pageNo}&pageSize=${this.list.pageSize}`, (j: ApiResponseResult) => {
    ...
    deleteInfo(id: number, index: number): void {
      this.list.loading = true;
      del(`${this.apiUrl}/${id}`, List.afterDelete(() => {
